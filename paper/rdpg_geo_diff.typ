...
#align(center)[
  #block(width: 85%)[
    *Abstract* \
    ...
    We develop a geometric framework based on principal fiber bundles that formalizes these obstructions and reveals their interplay. For horizontal families, polynomial dynamics have trivial holonomy, while Laplacian dynamics satisfy an explicit non-commutativity criterion for nontrivial holonomy; in $d = 2$ this yields full restricted holonomy $"SO"(2)$, in general $d$ we provide a conditional full-holonomy criterion, and for $d >= 3$ the generic full $"SO"(d)$ statement remains conjectural. Cramér-Rao lower bounds show that the spectral gap controlling geometric difficulty simultaneously controls statistical difficulty, an inextricable duality.
...
We establish a concrete contrast among horizontal dynamics families: polynomial dynamics have commuting generators and trivial holonomy, so gauge alignment is purely a statistical problem; for Laplacian dynamics, we prove a local commutator criterion for nontrivial holonomy and a full restricted $"SO"(2)$ consequence in dimension $d = 2$, provide a conditional full-holonomy criterion in general $d$, and leave generic full $"SO"(d)$ for $d >= 3$ as a conjecture.
...
@sec:dynamics catalogs concrete dynamics families, analyzes their observable consequences through the Lyapunov equation, and formalizes the polynomial-vs-Laplacian holonomy contrast with explicit proved/conditional/conjectural status.
...
*Standing assumptions (default throughout).*
Unless a result states otherwise, we work under:
+ *(S1)* fixed dimensions with $n >= d >= 2$;
+ *(S2)* trajectories on a time interval where $X(t)$ remains in the interior of $cal(E)$ (hence $"rank"(X(t)) = d$ and $0 < P_(i j)(t) < 1$);
+ *(S3)* sufficient smoothness of trajectories/fields for the stated derivatives and Lie brackets;
+ *(S4)* for statistical bounds, the regularity assumptions are those listed in @prop:cramer-rao.
...
#figure(
  table(
    ...
    stroke: none,
    table.hline(),
    table.header([*Result*], [*Status*], [*Scope*]),
    table.hline(stroke: 0.5pt),
    [@thm:invisible], [Proved], [Full-rank latent states],
    [@prop:curvature-criterion], [Proved], [Horizontal symmetric families],
    [@prop:laplacian-holonomy], [Proved], [Local nonzero commutator criterion],
    [@cor:linear-fisher], [Proved], [Linear dynamics],
    [@prop:cramer-rao], [Proved], [Polynomial dynamics + regularity assumptions],
    [@thm:gauge-contamination], [Proved], [Symmetric-generator identifiability],
    [Full restricted holonomy via curvature span], [Conditional theorem], [Requires curvature-span hypothesis],
    [Finite-time rank criterion at base point], [Conditional proposition], [Sufficient condition for full restricted holonomy],
    [@conj:full-holonomy], [Conjecture], [$d >= 3$ generic full holonomy],
    table.hline()
  ),
  caption: [Result status map: proved results vs conditional criteria vs conjecture.]
) <tab:result-status>

We use @tab:result-status as the authoritative status legend for theorem claims throughout the manuscript.
...
We can express the mismatch through the shared-subspace assumption.
...
Under ODE dynamics on latent positions, the column space of $P^((t)) = X^((t)) X^((t))^top$ can evolve over time; for non-commuting families (notably Laplacian dynamics), eigenvectors rotate and a single invariant subspace is typically absent.
...
In short: joint embeddings enforce a shared subspace; ODE-driven temporal RDPGs need not have one (except in special commuting/symmetric regimes).
...
#proposition(title: "Measure Zero for Finite-Dimensional ODE Families")[
  Let $cal(F) = {f_theta : theta in Theta subset RR^p}$ be a finite-dimensional $C^1$ family of vector fields on $RR^(n times d)$, and assume each ODE $dot(X) = f_theta(X)$ has a unique solution on $[0,T]$ for every initial condition.
  Let $mu$ be a non-degenerate Gaussian measure on $C^k([0,T], RR^(n times d))$ with full support.
  Define
  $ cal(M)_cal(F) = {X(dot) : dot(X) = f_theta(X) "for some" theta in Theta}. $
...
#proposition(title: "Zero Posterior Mass Without an ODE-Constrained Prior")[
  Let $Pi_0 = mu$ be a prior on path space with $mu(cal(M)_cal(F)) = 0$.
  For observed data $y$, define a posterior via
  $ Pi(A | y) = (integral_A L(y | X) d mu(X)) / (integral L(y | X) d mu(X)) $
  with measurable likelihood $L$ and finite, positive normalizer.
  Then $Pi(cal(M)_cal(F) | y) = 0$ for every such dataset $y$.
]

#proof[
  The posterior is absolutely continuous with respect to $mu$, with Radon-Nikodym derivative proportional to $L(y | X)$.
  Therefore every $mu$-null set is posterior-null, in particular $cal(M)_cal(F)$.
]

Thus, unless the ODE constraint is built into the prior/model, Bayesian updating cannot assign positive posterior mass to exactly dynamically consistent trajectories.
Asymptotic concentration near (rather than on) $cal(M)_cal(F)$ depends on model misspecification and is a separate question.
...
Centroid circulation $dot(x)_i = (x_i - macron(x)) A$ with $A in so(d)$ decomposes as $dot(x)_i = x_i A - macron(x) A$: the first term is pure gauge (@thm:invisible), while the shared drift $-macron(x) A$ generally shifts dot products, so $dot(P)$ is typically nonzero except in special symmetric configurations.
However, $X^top dot(X) = C A$ where $C$ is the sample covariance of positions; this is symmetric only under additional algebraic constraints (e.g., commutation-compatible structure), so the dynamics are typically non-horizontal and the trajectory spirals through the fibers.
...
(their Theorem 6), and then prove consistency of the CMDS mirror estimate from the estimated distance matrix (their Theorem 7), providing a concrete estimation guarantee for
...
== Holonomy for horizontal dynamics families (proved/conditional/conjectural) <sec:holonomy-dynamics>
...
As summarized in @tab:result-status, this section contains:
+ proved local criteria and the $d = 2$ full restricted holonomy consequence,
+ a conditional theorem/proposition for general $d$,
+ and a conjectural generic full-holonomy extension for $d >= 3$.

In this section we make the mechanism explicit for two dynamics families.
The punchline is a commuting/non-commuting contrast:
polynomial dynamics use generators that commute along the trajectory (hence no curvature along that path, and no loops to begin with), while Laplacian-type generators can fail to commute; whenever the projected-commutator criterion is nonzero, curvature is positive and nontrivial holonomy becomes possible.
...
#proposition(title: "Generic Nonvanishing of Projected Skew Action")[
  Let $n >= d >= 2$ and let $S in so(n)$ be nonzero.
  Define
  $ cal(N)_S := {X in RR_*^(n times d) : X^top S X = 0}. $
  Then $cal(N)_S$ is a proper real-algebraic subset of $RR_*^(n times d)$.
  In particular, it has empty interior and Lebesgue measure zero.
] <prop:generic-projected-skew>

#proof[
  Each entry of $X^top S X$ is polynomial in the entries of $X$, so $cal(N)_S$ is algebraic.
  It is proper because $S != 0$ implies there exist indices $i != j$ with $S_(i j) != 0$.
  Choose $x_1 = e_i$ and $x_2 = e_j$; then $x_1^top S x_2 = S_(i j) != 0$, so for $X_0 = [x_1, x_2, x_3, ..., x_d]$ with remaining columns chosen to make full column rank, $(X_0^top S X_0)_(1 2) != 0$.
  Hence $X_0$ is outside $cal(N)_S$, so $cal(N)_S$ is proper.
  Proper real-algebraic subsets of Euclidean space have empty interior and measure zero.
]

Thus $X^top [M_1, M_2] X = 0$ is strictly weaker than $[M_1, M_2]=0$, and for fixed nonzero commutator it fails only on an algebraic exceptional set in $X$.
Intersecting that exceptional set with the interior of the valid domain $cal(E)$ preserves this interpretation: on the interior model domain, the failure set remains proper.
...
Regular graphs (and more generally, configurations with constant row sums of $P$, i.e. $P bold(1) = c bold(1)$) thus have trivial holonomy under Laplacian dynamics.
...
#proposition(title: "Laplacian Dynamics: Local Commutator Criterion for Nontrivial Holonomy")[
  Let $X(t)$ solve Laplacian dynamics $dot(X) = -L(P) X$, with $P = X X^top$ and $L = D - P$.
  Suppose there exists $t_* in [0,T)$ such that
  $ C_* := X(t_*)^top [L(t_*), dot(L)(t_*)] X(t_*) != 0. $
  Then there exists $epsilon_0 > 0$ such that for all $0 < |h| < epsilon_0$,
  $ X(t_*)^top [L(t_*), L(t_* + h)] X(t_*) != 0. $
  Consequently, by @prop:curvature-criterion, the sectional curvature along the trajectory is strictly positive on that local two-time span.
  In particular, there exist contractible loops in a neighborhood of that region whose restricted holonomy is nontrivial.
] <prop:laplacian-holonomy>

#proof[
  Since $L$ is differentiable in $t$, Taylor expansion gives
  $ L(t_* + h) = L(t_*) + h dot(L)(t_*) + O(h^2). $
  Therefore
  $ [L(t_*), L(t_* + h)] = h [L(t_*), dot(L)(t_*)] + O(h^2). $
  Left and right multiplication by the fixed matrix $X(t_*)^top$ and $X(t_*)$ yields
  $ X(t_*)^top [L(t_*), L(t_* + h)] X(t_*)
    = h C_* + O(h^2). $
  Because $C_* != 0$, continuity implies there exists $epsilon_0 > 0$ such that this expression is nonzero for all $0 < |h| < epsilon_0$.

  Apply @prop:curvature-criterion with $M_1 = -L(t_*)$ and $M_2 = -L(t_* + h)$ (both symmetric): nonzero projected commutator implies strictly positive sectional curvature for the corresponding two-time span.
  The final holonomy claim follows from Ambrose--Singer: nonzero curvature implies nontrivial restricted holonomy, hence existence of contractible loops with non-identity holonomy.
]

The criterion above is checkable pointwise along a trajectory. It is also generic at initialization.

#proposition(title: "Genericity of the Local Commutator Condition at Initialization")[
  Fix $n >= 3$ and $2 <= d <= n$, and define
  $ Psi(X) := X^top [L(X), dot(L)(X)] X $
  with $P = X X^top$, $D = "diag"(P bold(1))$, $L = D - P$, $dot(P) = 2 P^2 - D P - P D$, and $dot(L) = "diag"(dot(P) bold(1)) - dot(P)$.
  Then
  $ cal(Z) := {X in RR_*^(n times d) : Psi(X) = 0} $
  is a proper real-algebraic subset of $RR_*^(n times d)$.
  Consequently, $cal(Z)$ has empty interior and Lebesgue measure zero; equivalently, for generic initial conditions, the hypothesis of @prop:laplacian-holonomy holds at $t_* = 0$.
  Restricting to valid RDPG states, the same conclusion holds after intersecting with $cal(E)$: this remains a proper exceptional set in the interior model domain.
] <prop:laplacian-generic-init>

#proof[
  Each entry of $Psi(X)$ is a polynomial in the entries of $X$ (all operations are sums and products), so $cal(Z)$ is algebraic.
  To show it is proper, it suffices to exhibit one full-rank $X$ with $Psi(X) != 0$.

  For $n = 3$, $d = 2$, take
  $ X_0 = 1/3 mat(1, 1; 2, 1; 2, 2). $
  A direct substitution into the formulas above gives
  $ (Psi(X_0))_(1 2) = 2 / 3^7 != 0, $
  hence $Psi(X_0) != 0$.

  For general $n >= 3$ and $2 <= d <= n$, embed this $2$-dimensional witness into the first two columns and first three rows, set remaining entries to zero, and then add an arbitrarily small perturbation in columns $3, ..., d$ making the matrix full rank.
  Since $Psi$ is continuous and nonzero at the embedded witness, for sufficiently small perturbation the value remains nonzero.
  Therefore a full-rank witness exists for every $n >= 3$ and $2 <= d <= n$, so $cal(Z)$ is a proper algebraic subset.

  A proper real-algebraic subset of Euclidean space has empty interior and Lebesgue measure zero.
]

For $d = 2$, the result is particularly sharp.

#corollary(title: "Dimension 2: Full Restricted Holonomy from One Nonzero Curvature Value")[
  Let $d = 2$ and assume the hypothesis of @prop:laplacian-holonomy holds at some time $t_*$.
  Then the restricted holonomy group of the bundle is $"SO"(2)$.
  Equivalently, for each angle $phi in [0, 2 pi)$ there exists a contractible loop in $cal(B)$ whose horizontal transport has holonomy rotation by $phi$.
]

#proof[
  The Lie algebra $frak(s o)(2)$ is one-dimensional, spanned by the single generator $J = mat(0, -1; 1, 0)$.
  By @prop:laplacian-holonomy, the curvature 2-form produces a nonzero element of $frak(s o)(2)$.
...
#remark[
  *Heuristic quantitative estimate (dimension 2).*
  For $d = 2$, the holonomy around a loop $gamma$ in $cal(B)$ is often approximated by an area integral of sectional curvature:
  ...
  This is an intuition aid, not a theorem used elsewhere in the manuscript.
  Combined with @massart2019curvature, it suggests that trajectories passing near rank-deficient states (small trailing eigenvalues) may accumulate larger holonomy.
]
...
#theorem(title: "Full Restricted Holonomy from a Curvature-Span Condition")[
  Let $d >= 2$ and fix a base point $p in cal(B)$.
  Suppose the Lie algebra generated by curvature values reachable from $p$ spans all of $frak(s o)(d)$, i.e.
  $ "span"{ "Ad"_(tau_(q -> p)) (Omega_q(u, v)) : q "horizontally reachable from" p,~u,v in cal(H)_q } = frak(s o)(d). $
  Then the restricted holonomy group at $p$ is:
  $ "Hol"_p^0 = "SO"(d). $
]

#proof[
  By the Ambrose--Singer theorem, the Lie algebra of $"Hol"_p^0$ is exactly the span above.
  Under the hypothesis, $"Lie"("Hol"_p^0) = frak(s o)(d)$.
  The group $"Hol"_p^0$ is connected and lies in $O(d)$; hence it lies in the identity component $"SO"(d)$.
  A connected Lie subgroup of $"SO"(d)$ with Lie algebra $frak(s o)(d)$ must be $"SO"(d)$ itself.
]

The theorem isolates the exact missing ingredient for $d >= 3$: a rank condition on curvature-generated directions.

#proposition(title: "Finite-Time Rank Criterion at a Fixed Base Point")[
  Let $X(t)$ solve $dot(X) = -L(P) X$ and fix $t_0$ such that $P(t_0)$ has rank $d$.
  Suppose there exist times $s_1, ..., s_m$ with $m = d(d-1)\/2$ such that the skew matrices
  $ B_a := X(t_0)^top [L(t_0), L(s_a)] X(t_0) in frak(s o)(d), quad a = 1, ..., m, $
  are linearly independent.
  Then, at the base point $p = [X(t_0)]$, the restricted holonomy group is $"SO"(d)$.
]

#proof[
  By @prop:curvature-criterion and @app:vertical-projection, each $B_a$ determines a curvature value at the same base point $p$ through the Lyapunov map
  $ B_a |-> Omega_a^*, $
  with componentwise scaling by positive factors $1\/(lambda_iota + lambda_gamma)$ (evaluated at $t_0$).
  This linear map on $frak(s o)(d)$ is invertible, so linear independence of $(B_a)$ implies linear independence of $(Omega_a^*)$.
  Since $m = dim(frak(s o)(d))$, these curvature values span $frak(s o)(d)$ at $p$.
  Therefore the curvature-span hypothesis of the previous theorem holds (with $q = p$, so no transport issue), and $"Hol"_p^0 = "SO"(d)$.
]

#remark[
  *Heuristic genericity note.*
  For Laplacian dynamics with $n >> d$, one expects the degree matrices at different times to make the finite-time rank criterion hold on most trajectories.
  Turning this into a theorem for all $d >= 3$ requires a transversality argument controlling symmetry strata.
  We therefore keep the fully generic statement as a conjecture.
]
...
A proof of @conj:full-holonomy would amount to proving the rank criterion above generically.
Equivalently, one must show that the projected commutators
$ X^top [L(P(t_1)), L(P(t_2))] X $
span $frak(s o)(d)$ for generic trajectories.
The main obstruction is structural symmetry: if the degree operator $P |-> D(P)$ preserves a proper invariant subspace in $frak(s o)(d)$, the generated holonomy algebra can collapse to a proper subalgebra.
Regular/equitable families are canonical examples of such degeneracy.
...
*Summary:* the holonomy obstruction creates a clear contrast among horizontal dynamics families.
...
Laplacian and message-passing dynamics, which mix spectral and spatial structure through non-commuting generators, can produce nontrivial holonomy whenever the projected-commutator criterion of @prop:curvature-criterion is triggered: gauge drift then accumulates even along horizontal trajectories, and no local alignment procedure can achieve global consistency over cycles.
...
The eigenvector rotation contributes additional degrees of freedom to the sensitivity, but this information is entangled with gauge degrees of freedom that may fail to be locally resolvable when the nonzero-curvature criterion of @prop:laplacian-holonomy is met.
...
#proposition(title: "Cramér-Rao Bound for Polynomial Dynamics")[
  For polynomial dynamics of degree $K$ with parameter $theta = (theta_0, ..., theta_K) in RR^(k)$ ($k = K + 1$), assume:
  + $theta$ belongs to an open parameter set $Theta subset RR^k$, and the true parameter is an interior point.
  + For each $t, i, j$, the map $theta |-> P_(i j)(t; theta)$ is $C^1$, with $0 < P_(i j)(t; theta) < 1$ on a neighborhood of the true parameter.
  + The score is integrable and differentiation can be interchanged with expectation (standard Fisher regularity), so the information identity is valid.
  + $cal(I)(theta)$ is positive definite on the identifiable parameter subspace.

  Then any unbiased estimator $hat(theta)$ satisfies:
...
#proof[
  ...
  *On the cross-eigenvalue terms.*
  ...
  In structured RDPGs (e.g., stochastic block models with equal-sized blocks), these weights are constant within blocks and the blocks are defined by the eigenvectors themselves, so the weights correlate with eigenvector components and cross terms $C_(iota gamma)$ for $iota != gamma$ can be nonzero (and are nonzero in explicit blockmodel configurations).
  ...
  The baseline bound $(cal(I)^(-1))_(a a) >= 1\/cal(I)_(a a)$ is immediate from Cauchy-Schwarz: $1 = (e_a^top e_a)^2 <= (e_a^top cal(I) e_a)(e_a^top cal(I)^(-1) e_a) = cal(I)_(a a) (cal(I)^(-1))_(a a)$.
...
If $cal(I)(theta)$ is singular, the same derivation applies on the identifiable subspace using the Moore-Penrose pseudoinverse $cal(I)^+$.
...
The connection 1-form involves factors $1 \/ (lambda_iota + lambda_gamma)$ controlling gauge sensitivity (@sec:fiber-bundle); the vertical bracket norm in sectional curvature contains the same denominators (weighted as in @prop:curvature-criterion), so weak eigendirections become geometrically delicate when commutator energy lies in those directions and $lambda_(d-1), lambda_d$ are small; and the Fisher information for estimating $M_(iota gamma)$ from the Lyapunov equation $dot(P)_(iota gamma) = (lambda_iota + lambda_gamma) M_(iota gamma)$ involves the same factor $(lambda_iota + lambda_gamma)^2$.
...
The additional eigenvector sensitivity contributes positively to the Fisher information (there are more directions in which observations carry signal), but it also introduces holonomy: the extra information can be entangled with gauge degrees of freedom in regions where @prop:laplacian-holonomy applies.
...
#remark[
  *Sampling-averaging heuristic.*
  When multiple independent network samples $A_1^((t)), ..., A_m^((t))$ are available at each time $t$, averaging
  $ macron(A)^((t)) = 1/m sum_i A_i^((t)) $
  reduces per-entry variance by a factor of $m$.
  In standard dense perturbative regimes, one therefore expects roughly a $sqrt(m)$ improvement in per-vertex embedding error.
  We use this as a practical scaling heuristic, not as a proved rate in this manuscript.
]
...
The analysis of concrete dynamics allowed us to identify a robust holonomy contrast (@sec:holonomy-dynamics): polynomial dynamics act through commuting generators (trivial holonomy along trajectories), whereas Laplacian dynamics satisfy a proved local nontrivial-holonomy criterion and, in $d=2$, a full restricted holonomy $"SO"(2)$ consequence; for $d >= 3$, we give a conditional full-holonomy criterion and keep the generic full-holonomy statement conjectural.
...
Assume $cal(F)$ is a finite-dimensional family with $p$ parameters (e.g., polynomial dynamics with $p = K+1$ coefficients), and restrict attention to a parameter domain on which solutions exist on $[0,T]$ and depend smoothly on parameters and initial conditions (standard under local Lipschitz conditions in $X$ and smoothness in parameters; see Lee, _Introduction to Smooth Manifolds_, 2nd ed., 2013, book-level reference).
For each $(f, X_0) in cal(F) times RR^(n times d)$, the Picard-Lindelöf theorem yields a unique solution $X(dot; f, X_0) in C^k([0,T], RR^(n times d))$ (same reference).
...
A standard consequence of Sard's theorem (equivalently, the area formula) is that the image of such a map has Lebesgue measure zero in $RR^(m+1)$ (book-level reference: Milnor, _Topology from the Differentiable Viewpoint_, 1997 ed., Sard theorem section).
